...
public:
  ...
  size_t getValueAsText(char* buffer, size_t buflength){return 0;}; ///<  write value as a human readable text, with at most buflength characters
   size_t  getValueTextLength(){return 0;}; ///< get size of the buffer needed for getValueAsText()
  ...
  // same here: these methods are primarily inteded to make interfaceing simple, imeplement them as you think they make sense
   float getValueAsFloat(size_t position=0){return 0;};
   int32_t getValueAsInt32(size_t position=0){return 0;};
   bool getValueAsBool(size_t position=0){return 0;};
   double getValueAsDouble(size_t position=0){return 0;};
  ...
  ARDUPAR_CONST_CHAR * getDescription(){return descriptionString;}; ///< a human readable description of the parameter
   ArduPar3Type getType(size_t position=0){return position==0? AbstractArduPar3::ArduPar3TypeTrigger:AbstractArduPar3::ArduPar3TypeNone;}; ///< to aid building of user interfaces at runtime
protected:
  size_t uniqueId;
